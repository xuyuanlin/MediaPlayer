...
public class VisualizerView extends View {
	...
	public void setupVisualizerFx(int audioSessionId) {
		Flog.d("-----VisualizerView----setupVisualizerFx()");
	...
	private void updateVisualizer(byte[] mbyte) {
		// Flog.d("VisualizerView-----updateVisualizer()");
		/*
		 * bytes = mbyte; invalidate();
		 */
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < mSpectrumNum; i++) {
			...
			if (canDrawLines) {
				...
				linesArray[i * 4 + 3] = height - mBytes[i] * 2;// 终点Y坐标
				// linesArray[i * 4 + 3] = height - 10;// 终点Y坐标

			}

			// 线终点位置的高度减去一个数就会出现在线的上方，反之在下方
			int y = height - mBytes[i] * 2 - 3;
